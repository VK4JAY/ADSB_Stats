...
try {
    ...
    $stmt = $pdo->prepare("SELECT DISTINCT route FROM flights WHERE src_country = '$userCountryCode' AND dst_country = '$userCountryCode' AND route != '' ");
    $stmt->execute();
    ...
    $stmt = $pdo->prepare("SELECT iata_code, latitude, longitude, municipality FROM airports WHERE iata_code IN ($placeholders)");
    ...
    foreach ($airports as $airport) {
        ...
        $airport_details[$iata_code] = $airport['municipality'];

        $result[] = [
            'id' => $airport['municipality'],
        ...
        if (in_array($iata_code, $source_airports)) {
            $municipality = preg_replace('/\s+/', '', $airport['municipality']); // Remove spaces from municipality name
            $source_airports_js_parts[] = "{$municipality}Point = chart.get(\"{$airport['municipality']}\")";
        }
    }
